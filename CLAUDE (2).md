...
## Sesión 18 — Landing Hero: eliminar Globe3D, crear HeroArc

Tarea ejecutada en modo autónomo. El globo 3D nunca fue terminado de pulir (océanos negros, arquitectura compleja, conceptualmente redundante con WorldMapPublic.tsx) — decisión del fundador: eliminarlo y reemplazarlo por un elemento geométrico propio derivado del anillo rojo del logo oficial, enmarcando la foto real del Colegio Albania.

- **Eliminado por completo**: `Globe3DHero.tsx`, `Globe/Globe3DCanvas.tsx`, `Globe/Globe3DWorker.ts`, `Globe/Globe3DFallback.tsx`, `public/textures/earth-new/` (earth-day.jpg + earth-night.jpg — solo los usaba Globe3DWorker.ts). Conservados: `Globe/GlobeCanvas.tsx`, `Globe/GlobeWorker.ts`, `Globe/GlobeFallback.ts` — los usa `WorldMapPublic.tsx`. Confirmado vía `grep -r "Globe3D" src/`: cero resultados.
- **`HeroArc.tsx`** — composición editorial con grid `1fr 1.1fr` (desktop) / `1fr` (mobile <768px). Columna izquierda: eyebrow pill rojo (Satoshi 10px uppercase tracking), H1 con last line en Instrument Serif italic rojo ("global e innovador"), subcopy 38ch muted, botón pill con inner arrow circle, micro-stat 876 en Instrument Serif italic. Columna derecha: dos arcos SVG independientes con `strokeLinecap="butt"` — arco 1 rojo (`var(--accent)`, 13px, ~62% de circunferencia), arco 2 ink bajo opacity (9px, ~40%), ambos con draw-in vía FM `pathLength` 0→N desde `whileInView`; foto circular del colegio (object-fit:cover, `filter:saturate(0.85) contrast(1.05)` en CSS estático) dentro del arco; badge pill "Sede · Colegio Albania" fuera del overflow:hidden del crop circular.
- **Mounted guard para scroll parallax**: `const { scrollYProgress } = useScroll({ target: heroRef })` + `const rightY = useTransform(scrollYProgress, [0,1], [0,-48])` — aplicado como `style={mounted ? { y: rightY } : undefined}` en la columna derecha. El hook se llama incondicionalmente (regla de React), pero el MotionValue solo se aplica al DOM después del mount para evitar hydration mismatch.
- **`pathLength` en FM para draw-in de arcos**: la alternativa `strokeDashoffset` directamente no funciona correctamente en paths cerrados (el offset "rota" el arco en vez de "dibujarlo"). FM's `pathLength: 0 → 0.62` normaliza internamente a `strokeDasharray`/`strokeDashoffset` para paths abiertos y cerrados. La rotación de origen de cada arco se hace con `<g transform="rotate(N cx cy)">` estático (no FM) envolviendo el `m.circle` — FM solo anima `pathLength`, sin conflicto.
- **Foto — fallback por bucket privado**: la URL pública `…/storage/v1/object/public/campus-photos/colegio-albania-entrada.jpg` falla si el bucket no es público (devuelve 403). El handler `onError={() => setImgError(true)}` activa `.ha-photo-ph` (placeholder `var(--bg-2)`) sin romper el layout — los arcos siguen encuadrando el área vacía. Para mostrar la foto real, el bucket `campus-photos` debe ser marcado como público en el panel de Supabase Storage.
- **GlobeHero.tsx**: eliminados `import dynamic`, `import AnimatedNumber`, `import { useRealtimeStats }`, la constante `Globe3DHero`, los componentes `CountryScramble`/`SCRAMBLE_WORDS`, el estado `scrollHintVisible` y su efecto, `liveStats`/`statsLoading`, y el bloque JSX completo de `<section className="hero">`. Reemplazado por `<HeroArc />`. El CSS de la sección `.hero` quedó huérfano en el `<style>` inline (no borrado para no arriesgar el string de 370 líneas — no genera errores).
- **i18n**: nuevas keys `landing.heroArc.{eyebrow,title,titleAccent,subtitle,cta,studentCount,studentCountLabel,photoBadge}` en los 5 locales (es/en/fr/pt/ar.json). Todas validadas con `node -e "JSON.parse(...)"` → OK.
- **Verificación visual**: Playwright + Chrome del sistema, `http://localhost:3000/`. Desktop: eyebrow pill, H1 Satoshi bold + Instrument Serif italic rojo, arco rojo ~62% + arco gris ~40%, placeholder beige cirucular, badge "Sede · Colegio Albania" totalmente visible (fix: mover badge fuera del overflow:hidden del photo-wrap). Mobile: arco aparece arriba, texto abajo. Cero Globe3D en el DOM. El único error de consola es un CSP header preexistente no relacionado con estos cambios.