...
export default function CoordinatorReportPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      evalRows?.forEach((e: { project_id: string; resultado: string | null; projects: { user_id: string } | null }) => {
  ...
  async function handleExport() {
    ...
    try {
      ...
      doc.text('Big Family', 14, 18)
      doc.setFontSize(11)
      doc.setTextColor(100)
      doc.text(`Reporte de estudiantes — ${schoolName}`, 14, 26)
      doc.text(`Exportado: ${new Date().toLocaleDateString('es-CO', { day: '2-digit', month: 'long', year: 'numeric' })}`, 14, 32)
      ...
      const certified = students.filter(s => s.capstone_resultado !== null).length
      const avgXp = students.length > 0 ? Math.round(students.reduce((s, r) => s + r.total_xp, 0) / students.length) : 0
      doc.setFontSize(10)
      doc.setTextColor(60)
      doc.text(`Total estudiantes: ${students.length}   |   Certificados: ${certified}   |   XP promedio: ${avgXp}`, 14, finalY)

      doc.save(`reporte-${schoolName.toLowerCase().replace(/\s+/g, '-')}-${new Date().toISOString().split('T')[0]}.pdf`)
    } catch (err) {
