...
- **The Great Venture (Mapa Hoshin Kanri)** — Wizard de 5 pasos + mapa radial SVG animado. `great_ventures` table (meta_nucleo, creencias, paradigma, equipo JSONB, planes JSONB). Wizard fullscreen overlay (z-index 200) con segmented progress bar + AnimatePresence x-transitions + autosave debounce 800ms + títulos adaptativos por arquetipo de líder. Mapa: dark bg, SVG radial con 4 satélites (Creencias/Paradigma/Planes/Equipo), pathLength animation para líneas, scale spring para nodos, panel deslizable x:360→0 para edición in-situ, export PNG via html2canvas. AppSidebar: ícono compass + badge "✓"/"Pendiente" dinámico. StudentProfileClient: columna "Great Venture" con estado. MOCK_MODE completo.
