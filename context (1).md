...
### Fix real de overlap: el botón X tapaba texto y "Ejemplo" tocaba el borde — el fix de padding de la Sesión 28 no alcanzó (Sesión 29)

Reporte con captura ("El Conector"): la palabra "hacer" del párrafo de descripción quedaba oculta debajo del botón X, y el bloque "Ejemplo" seguía pegado al borde derecho del panel. Se pidió diagnóstico antes de tocar código y verificación medida (no solo visual) en los 5 arquetipos, porque el bug de la Sesión 28 ya había pasado una revisión que solo cubrió 1-2 casos.

**Diagnóstico con Playwright (`boundingBox()` de `.sp-panel__close`, `.sp-panel__desc`, `.sp-panel__ej`, `.sp-panel` en los 5 arquetipos, no solo capturas)**: reveló que el problema real no era "poco padding cerca del borde" sino dos bugs de layout, presentes en los 5 arquetipos por igual, no solo en el de párrafo más largo:

1. **Causa raíz del "Ejemplo" pegado al borde**: `.sp-panel__inner{grid-template-columns:46% 54%}` — en CSS Grid, los porcentajes en `grid-template-columns` se resuelven contra el ancho del contenedor *antes* de restar el `gap`, así que `46%+54%=100%` del contenedor más `gap:48px` suman más ancho del que el contenedor tiene, y el grid no encoge las pistas para compensar. Medido: la columna derecha desbordaba el borde derecho del propio panel por ~3px en los 5 casos (`ej right edge to panel right edge gap: -2.98`, negativo = desborde). Fix: cambiar a `grid-template-columns:46fr 54fr` — las unidades `fr` sí restan el `gap` del espacio disponible antes de repartir, por spec. Con el fix, el hueco pasó a ser consistentemente +45px en los 5 arquetipos (desktop) y +21px simétrico en mobile.
2. **Causa raíz del botón X tapando texto**: `.sp-panel__close` es `position:absolute;top:20px;right:20px` dentro de `.sp-panel`, sin que `.sp-panel__right` reservara espacio para su huella (40×40px). El botón flotaba sobre el flujo del texto en vez de tener su propia franja — la esquina superior derecha de `.sp-panel__desc` caía directamente bajo el botón en los 5 casos (confirmado por `boundingBox` overlap: true en los 5, no solo en el arquetipo de párrafo largo — el desborde del bug #1 empeoraba esto al desplazar la columna derecha aún más a la derecha, hacia la zona del botón). Fix: `padding-top:24px` en `.sp-panel__right`, aplicado solo en `@media(min-width:961px)` (el layout de dos columnas) — en mobile las columnas se apilan y `.sp-panel__right` ya arranca muy por debajo del botón, así que el padding extra ahí sería innecesario.

**Verificado con medición programática, no solo visual**, en los 5 arquetipos, desktop y mobile: `boundingBox` de `.sp-panel__close` vs `.sp-panel__desc` sin overlap en los 10 casos (5×2 viewports); gap `.sp-panel__ej`-borde positivo y consistente en los 10 casos. Confirmado también por captura de página completa que "VÍNCULO" sigue sin cortarse (el fix de columnas de la Sesión 28 no se tocó). `npx tsc --noEmit` limpio.

**Lección**: verificar overlap/padding solo con capturas visuales de 1-2 casos no detecta bugs de layout que dependen del contenido (longitud de párrafo) o que solo se ven en desbordes de unos pocos px — medir `boundingBox()` programáticamente en todos los casos releva la causa raíz real (un bug de CSS Grid, no "falta de padding") y confirma la ausencia del bug con certeza, no con inspección ocular.

### Rediseño de la sección Valores — de ícono placeholder a jerarquía tipográfica en las cards 01+02 (Sesión 30)

Reporte: los cuadros de ícono placeholder (div beige vacío ~64×64px, `.sec-valores__img-ph`) en las 6 cards de `/metodologia#valores` nunca tuvieron diseño definido.

**Discrepancia encontrada antes de tocar código**: el JSX (`metodologia/page.tsx`) solo marcaba `i === 0` como `--featured` (grid-column:span 2) — solo "Ética" era grande; "Compromiso" ya tenía el mismo tamaño que 03-06. El pedido original describía 01 y 02 como grandes por igual. Confirmado con el fundador antes de implementar: extender featured a `i < 2`, con el grid reestructurado a `repeat(4,1fr)` — así las dos cards featured (`span 2` cada una) ocupan una fila completa entre las dos (50/50), y las 4 chicas (`span 1` cada una) llenan la fila siguiente en una sola línea de 4.

**Primer intento (descartado por el fundador)**: reemplazar el ícono por un número tipográfico watermark gigante detrás del título (Instrument Serif italic, `opacity:.07` sobre `var(--ink)` para seguir el tema, mismo lenguaje que `.sec-historia__watermark-yr`). Feedback directo: se sentía genérico/decorativo, no resolvía el vacío real del espacio extra en las cards grandes.

**Fix real (Sesión 30, segundo intento)**: sin elemento decorativo nuevo — el espacio extra de las cards grandes se llena con tipografía escalada y más aire, no con un adorno de fondo. `.sec-valores__watermark` eliminado del CSS y del JSX por completo (junto con el `position:relative;overflow:hidden`/`z-index` que existía solo para contenerlo). Cambios en `.sec-valores__tile--featured`: `padding` de `32px 28px` a `48px 44px`; `.sec-valores__name` de `17px` fijo a `clamp(28px,2.6vw,36px)` (equivalente a pasar de ~text-lg a text-3xl/4xl); `.sec-valores__desc` de `14px` a `16.5px` con `line-height:1.75` (antes 1.6); gap interno del contenido de `12px` a `18px`. En el breakpoint `≤600px` (mobile, 1 columna), tamaño intermedio para las featured — `name:24px`, `desc:15px`, `padding:36px 28px` — más grande que las cards chicas pero sin el tamaño desktop completo, que se vería desproporcionado en una card de ancho completo angosta. El label pequeño "01"/"02" (`.sec-valores__num`, 11px, `var(--accent)`) y la micro-cita en Instrument Serif italic con su separador (`.sec-valores__quote`, agregada en el intento anterior) se mantuvieron sin cambios — esa parte del rediseño anterior sí funcionó y el fundador pidió conservarla explícitamente.

**Verificado**: `boundingBox()` de las 6 tiles confirma que las featured ahora son notablemente más altas que las chicas (308-316px vs 199-177px, resultado directo de la tipografía + padding, no de contenido extra) — se sienten como "declaración de marca" en vez de la misma card con más aire vacío. Capturas claro/oscuro confirman la jerarquía sin elementos compitiendo por atención. Hover (`whileHover={{y:-4}}` + color de título a accent) y el blur-reveal stagger de scroll (`whileInView` con `filter:blur()`) sin cambios, confirmados intactos. Mobile confirma la escala intermedia — el título de Ética/Compromiso sigue destacando sobre 03-06 sin ocupar toda la card. `npx tsc --noEmit` limpio.

**Nota para la próxima vez que se pida un elemento decorativo de relleno**: la lección de esta sesión es que "llenar espacio vacío" y "dar jerarquía real" no son lo mismo — un watermark o ícono grande ocupa el área pero no comunica que esa card pesa más que las otras; escalar la tipografía real sí, porque el lector percibe el tamaño del contenido mismo, no un adorno de fondo.

### Diploma en /certificacion/[id] — el arquetipo NO estaba perdido, y pulido "premium/ceremonial" (Sesión 31)

**Parte 1 — investigación del arquetipo "perdido"**: el pedido llegó como "confirmado por el fundador" que el arquetipo se perdió o nunca se implementó. Antes de tocar código, se auditó: (1) el JSX de `certificacion/[id]/page.tsx` ya trae `leadership_profile` en el `select()` de `profiles`, ya renderiza `data.leaderProfile.arquetipo` en Instrument Serif italic accent debajo del nombre, y ya lo oculta con gracia (`data.leaderProfile &&`) sin dejar hueco cuando es `null`; (2) `git log` confirma que esto se implementó en el commit `c797abc feat: diploma con arquetipo + QR verificable + página /verify pública`, ya documentado en la línea "Diploma con arquetipo + QR verificable" de este archivo; (3) `git merge-base --is-ancestor c797abc origin/main` confirma que ese commit **ya está desplegado en main/producción**, no es trabajo pendiente. Se verificó además con la service role key (consulta de solo lectura) que la tabla `profiles` en producción tiene **0 filas** — consistente con el reset completo de la Sesión 24 — así que ningún estudiante real ha tenido jamás un diploma con datos reales para comparar. Conclusión: el código nunca se perdió; lo más probable es que el fundador recordara el pedido original sin haber vuelto a verificar el estado actual, o que lo haya visto antes del commit `c797abc`. **No se reimplementó nada de la Parte 1** — reimplementar código que ya funciona correctamente habría sido trabajo duplicado y riesgo innecesario. En su lugar se verificó el comportamiento con datos mock controlados (ver abajo) para confirmar con capturas reales, tal como se pidió.

**Parte 2 — pulido visual, decisiones de diseño evaluadas una por una** (no las 6 sugerencias se aplicaron a ciegas):

1. **Doble borde**: ya existía (`::before` outer 2px + `::after` inner 1px, gap de 6px) pero el gap era casi imperceptible. Aumentado a outer `3px` + inner con `inset:14px` (gap ~11px) y padding del card de `48px 72px` a `60px 76px` para dar aire al marco — ahora sí se lee como doble marco, no como un borde grueso con una línea pegada al lado.
2. **Nombre del estudiante**: `clamp(2.2rem,5vw,3.6rem)` → `clamp(2.5rem,6vw,4.4rem)`, `line-height` 1.1→1.08. Domina la composición con más autoridad sin romper el layout en los nombres largos probados.
3. **"THE BIG LEADER"**: pasó de texto suelto a un bloque flex con dos líneas ornamentales (28px, `rgba(192,57,43,.35)`) flanqueando el texto — tratamiento clásico de certificado ("— THE BIG LEADER —"), y el tamaño subió de 15px a 16px con tracking de `.22em` a `.26em`. Confirmado que las líneas no rompen a una segunda fila incluso en mobile 390px.
4. **Stats (Puntos de Impacto / Módulos)**: número de `28px`→`38px` (siguiendo el mismo principio que `.kpi-num` del dashboard, 32px número vs 10.5px label) para más contraste con el label de 10px. Al subir el tamaño el row de stats+validaciones se rompía a dos líneas por el ancho extra — corregido reduciendo `.dp-stats-cell` padding de `28px` a `24px` por lado para que las 3 celdas (XP · Módulos · Reconocido por) sigan cabiendo en una sola fila con sus separadores verticales, en vez de aceptar el wrap accidental.
5. **Sello circular**: se agregó un anillo de "cuentas" (`BeadRing`, 40 puntos pequeños en `r=43.5` entre los dos anillos existentes), un tercer anillo interno tenue (`r=24`, opacity .4) alrededor de la estrella, y un `radialGradient` sutil de fondo — mismo lenguaje de "sello de cera" sin volverse un elemento nuevo ajeno al resto. Tamaño de 84px a 94px.
6. **Espaciado general**: aumentado el aire entre secciones — "Este certificado se otorga a" `marginTop` 28→36, párrafo "por haber completado" `marginBottom` 10→16, colegio/fecha 6→8 y 28→34, stats row `margin` 22px→30px, firma row `marginTop` 22→32. El documento "respira" más sin perder que todo siga cabiendo en una página al imprimir.

**Fix adicional encontrado durante la verificación de impresión (fuera del pedido original pero directamente relacionado)**: el confetti (`PARTICLES.map`) nunca tuvo clase `.no-print` — solo los botones la tenían — así que las partículas de colores aparecían sueltas debajo de la tarjeta en la vista de impresión/PDF. Se agregó `.dp-confetti` a cada partícula y `.dp-confetti{display:none!important}` dentro de `@media print`. Confirmado con `page.emulateMedia({media:'print'})` + `page.pdf()`: PDF de una sola página, sin confetti, sin botones, sombra del card removida.

**Verificado**: capturas con `leaderProfile` de dos arquetipos distintos ("Líder Visionaria" y "El Guardián", vía hook de prueba temporal `studentId==='test-guardian'` en la rama `MOCK_MODE`, revertido antes de commitear) confirman que el texto no se ve forzado con nombres de distinta longitud; captura con `leaderProfile:null` (`studentId==='test-none'`) confirma que no queda hueco vacío — el `marginBottom` condicional del nombre (`data.leaderProfile ? 6 : 24`) compensa el espacio correctamente. Mobile 390px confirma que las líneas ornamentales de "THE BIG LEADER" y el resto del layout no se rompen. `npx tsc --noEmit` limpio.

### Existían 2 copias del diploma — extraído a `<DiplomaCard>` compartido (Sesión 32)

Reporte: la vista previa del diploma en la landing (botón "Ver ejemplo de diploma" en `/metodologia`, sección Certificación, modal `.dm-panel` en `GlobeHero.tsx`) no reflejaba nada del pulido de la Sesión 31 (doble borde más ancho, nombre más grande, líneas ornamentales en "THE BIG LEADER", stats con más contraste, sello con anillo perlado + glow). Investigación previa a tocar código: **no era un iframe ni un embed de `/certificacion/[id]`** — era una segunda copia completa, manual, del markup del diploma, escrita directamente en `GlobeHero.tsx` (membrete → firma/sello) con sus propios estilos inline hardcodeados, nunca conectada al componente real. Esta copia ni siquiera tenía el arquetipo (nunca lo tuvo — se escribió antes de la Sesión 23 que lo agregó al diploma real) ni el QR.

**Fix — extracción a componente compartido, no duplicar el pulido dos veces**:
- `src/lib/diploma.ts` (nuevo): tipos `LeaderProfile`/`DiplomaData` y los helpers puros `formatDate`, `certNumber`, `makeCertId`, `arquetipoCode` — antes vivían solo dentro de `certificacion/[id]/page.tsx`.
- `src/components/DiplomaCard.tsx` (nuevo): el componente visual completo del diploma (membrete → firma/sello/QR), incluyendo `Seal`/`BeadRing`. Props: `data: DiplomaData`, `certNumberText: string`, `qrDataUrl?`, `compact?: boolean` (tamaño reducido para la vista previa de landing — paddings, tipografía y el propio doble-bezel escalados hacia abajo con las mismas proporciones, no una versión distinta), `animated?: boolean` (stagger por elemento para la página real vs. render estático para el modal, que ya tiene su propia animación de entrada en `GlobeHero.tsx`). Un helper `fx()` devuelve `{}` cuando `animated=false`, así los mismos `m.p`/`m.div` funcionan como elementos estáticos sin duplicar JSX.
- `certificacion/[id]/page.tsx`: se eliminaron los tipos/helpers/`Seal`/`BeadRing` duplicados (ahora importados) y el bloque `.dp-card` completo (~310 líneas) se reemplazó por `<DiplomaCard data={data} certNumberText={certNumber(studentId,data.certDate)} qrDataUrl={qrDataUrl} animated />`. Se eliminó también `MiniPentagon` — estaba definida pero **nunca se renderizaba en ningún lado** (código muerto desde su creación en la Sesión 23, pese a que la nota de esa sesión en este archivo decía que sí se usaba "en la franja de validaciones"; otra discrepancia entre lo documentado y el código real, en la línea de la Sesión 31).
- `GlobeHero.tsx`: el modal (`.dm-panel`) se redujo a solo su chrome (posición, scroll, max-height) — el borde/padding/fondo del diploma en sí (antes duplicado en `::before`/`::after`) se eliminó de `.dm-panel` porque ahora lo provee `<DiplomaCard compact>`. Datos mock (`DIPLOMA_PREVIEW_DATA`, reutilizando `MOCK.mockDiploma` + un `leaderProfile` de ejemplo) reemplazan el nombre/stats/cert-number hardcodeados en JSX suelto.
- Bug encontrado y corregido durante la verificación en mobile: el modo `compact` no tenía ninguna regla `@media` propia — en viewports angostos (`≤420px`) las 3 celdas de stats y la fila de firma se envolvían de forma desordenada (separadores verticales flotando sueltos). Se agregó el mismo patrón de `flex-direction:column` que ya usa el modo completo, en su propio breakpoint más angosto acorde al tamaño del modal.

**Por qué esta es la solución correcta y no "copiar el pulido dos veces"**: con el componente compartido, cualquier pulido futuro al diseño del diploma (otro cambio de tipografía, otro ajuste al sello, etc.) se escribe una sola vez en `DiplomaCard.tsx` y automáticamente se refleja en ambos lugares — ya no puede volver a desincronizarse porque ya no hay una segunda copia que actualizar.

**Verificado**: captura de `/certificacion/[id]` después de la extracción es visualmente idéntica a la de la Sesión 31 (mismo doble bezel, tamaños, sello) — confirma que refactorizar no rompió el original. Captura del modal de landing confirma que ahora sí muestra el arquetipo ("Líder Visionaria"), el doble bezel, las líneas ornamentales de "THE BIG LEADER", los stats con más contraste y el sello con anillo perlado — mismo tratamiento visual que la página real, a escala compacta. Impresión/PDF de `/certificacion/[id]` reverificada después del refactor: una sola página, sin confetti, sin botones. Mobile confirmado en ambos (390px real page, ≤420px modal). `npx tsc --noEmit` limpio.
