...
### Rediseño de la sección Valores — watermark tipográfico y cards grandes 01+02 (Sesión 30)

Reporte: los cuadros de ícono placeholder (div beige vacío ~64×64px, `.sec-valores__img-ph`) en las 6 cards de `/metodologia#valores` nunca tuvieron diseño definido. Dirección aprobada: reemplazarlos por un número tipográfico watermark gigante detrás del título, mismo lenguaje "Editorial Luxury" que `.sec-historia__watermark-yr` (el "2015" watermark de la sección Historia).

**Discrepancia encontrada antes de tocar código**: el JSX (`metodologia/page.tsx`) solo marcaba `i === 0` como `--featured` (grid-column:span 2) — solo "Ética" era grande; "Compromiso" ya tenía el mismo tamaño que 03-06. El pedido original describía 01 y 02 como grandes por igual. Confirmado con el fundador antes de implementar: extender featured a `i < 2` (recomendado), con el grid reestructurado a `repeat(4,1fr)` — así las dos cards featured (`span 2` cada una) ocupan una fila completa entre las dos (50/50), y las 4 chicas (`span 1` cada una) llenan la fila siguiente en una sola línea de 4 — en vez del layout previo de 3 columnas donde solo una card grande dejaba un hueco de 1 columna al lado.

**Watermark**: `.sec-valores__watermark` — Instrument Serif italic (a propósito distinto del Satoshi 900 de Historia, pedido explícito de la dirección), `position:absolute` en la esquina superior derecha de cada tile, `color:var(--ink)` con `opacity:.07` a nivel de elemento (no un `rgba()` con negro fijo horneado como hace Historia) — esto es intencional: un rgba fijo se ve invisible sobre `--card-bg` oscuro en modo oscuro (mismo problema de fondo de la Sesión 25), mientras que `opacity` sobre `var(--ink)` invierte correctamente con el tema (ink claro sobre card oscura en dark mode, ink oscuro sobre card clara en light mode). Tamaño diferenciado: 104px en cards chicas, 168px en featured (96px/136px en el breakpoint ≤600px) — el número, no solo el ícono, ahora comunica jerarquía de tamaño. `.sec-valores__tile` pasó a `position:relative;overflow:hidden` (el overflow recorta el número donde se sale del borde, mismo tratamiento "sangrado" que el watermark de Historia) y el contenido real (num pequeño, nombre, desc, cita) se envolvió en `.sec-valores__content{position:relative;z-index:1}` para garantizar que el watermark (sin z-index propio, pintado en el paso 2 del modelo de stacking de CSS — stacking contexts con z-index negativo/`auto` en contenido no posicionado — antes que el contenido normal-flow del paso 3) quede siempre detrás sin necesidad de trucos de z-index negativo.

**Contenido diferenciado**: cards featured (Ética, Compromiso) ganaron un bloque `.sec-valores__quote` — micro-cita en Instrument Serif italic, separada por un `border-top` sutil, debajo de la descripción existente. **Copy marcado como borrador, pendiente de confirmación del fundador**: "La integridad no se negocia cuando nadie está mirando." (Ética) y "El compromiso se mide en los días difíciles, no en los fáciles." (Compromiso) — agregadas en los 5 locales (`landing.valores.etica.quote` / `.compromiso.quote` en es/en/fr/pt/ar), traducidas pero no verificadas por un hablante nativo del tono de marca en cada idioma. Cards 03-06 mantienen el formato sin cambios (sin cita forzada), tal como se pidió.

**Verificado**: `boundingBox()` de las 6 tiles confirma el grid 2-grandes-arriba + 4-chicas-abajo (592px/592px fila 1, 288px×4 fila 2 en desktop 1400px). Capturas claro y oscuro confirman el watermark legible pero sutil en ambos temas, sin competir con el título. Hover (`whileHover={{y:-4}}` + color de título a accent) y el blur-reveal stagger de scroll (`whileInView` con `filter:blur()`) sin cambios, confirmados intactos. Mobile (1 columna apilada, breakpoint ≤600px) confirma el watermark proporcional, sin desbordar ni empujar el texto. `npx tsc --noEmit` limpio.
